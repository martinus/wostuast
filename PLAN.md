...
| An ignored directory is listed when it is small enough to read | git collapses one into a single entry and looks no further, which is the right trade for `node_modules` and the wrong one for a directory an agent generates its plan into. So wostuast walks it, and keeps git's promise itself: a folder is listed only when what it holds fits in `IGNORED_MAX`, and one that does not is a single row saying so. The rule is recursive, so a build root of a hundred thousand objects is left out and the plan beside it is listed. Finding that out costs the budget in reads, not the directory's size. |
| A `links.json` that cannot be used is said out loud | `serve` prints it, the Session tab shows it, `doctor` says it. The file is the one thing here a person writes by hand, and the first thing anybody writes is `\d` — which is not a JSON escape, so the file never parses. Dropping it in silence made a file you wrote and got wrong look exactly like a file you never wrote. It is never repaired: guessing at the backslash somebody meant is a worse surprise than the message. |
...
| A send is capped in bytes, at a number tmux sets | tmux is what refuses a long `send-keys`, and it counts bytes: bisected against tmux 3.4, 16,341 of them, and half that in two-byte characters. A cap on `len()` is a cap on the wrong thing. It is refused rather than cut or split, because what arrives in the terminal must be what you wrote — and a bracketed paste broken across two sends leaves the rest arriving as keystrokes. |
...
| Two alert switches, not one | "An agent needs you" and "an agent has finished" are different news, and one switch means taking the one you do not want with the one you do. Needs-you is on as soon as alerts are, because it is what this tool is for. Both live in the browser, like the theme, so no config file. |
| The end of the tab row carries the context bar | Issue #101 asked for four things there. Context is the first, and the status line carries it. **This row said money was in no payload, and that was wrong** — see the row below, and `tests/fixtures/README.md`. Machine load is the daemon's machine, which is the agent's only until you open the page from somewhere else. A spend limit that *stops* an agent is the one of the four that stays refused: it means owning the agent process, which goal 3 rules out, and a tool that can kill your work is a different tool. |

...
| The end of the tab row carries the context bar, and nothing else | Issue #101 asked for four things there. Context is the one wostuast can know: the status line carries it. Money is in no hook or status-line payload, so a spend or an allowance would be invented. Machine load is the daemon's machine, which is the agent's only until you open the page from somewhere else. And a spend limit that stops an agent means owning the agent process, which goal 3 rules out — nothing here owns one, and a tool that can kill your work is a different tool. |

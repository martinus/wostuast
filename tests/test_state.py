...
def test_session_start_records_the_facts(ws):
    session = fold(ws, event(ws, "SessionStart", source="startup"))
...
def test_prompt_makes_it_working_and_is_kept(ws):
    session = fold(ws, event(ws, "SessionStart"), event(ws, "UserPromptSubmit", prompt="do it"))
...
def test_pre_and_post_tool_use_are_working(ws):
    session = fold(
        ws,
        event(ws, "PreToolUse", tool_name="Bash", tool_input={"command": "make"}),
    )
    assert session.state == "working"
    assert session.last_tool == "Bash make"
    session = fold(
        ws,
        event(ws, "PostToolUse", tool_name="Read", tool_input={"file_path": "/w/repo/dir/a.py"}),
    )
    assert session.state == "working"
...
def test_permission_notification_needs_you(ws):
    session = fold(
        ws,
        event(ws, "PreToolUse", tool_name="Bash", tool_input={"command": "cmake --build build"}),
        event(ws, "Notification", notification_type="permission_prompt", ts=1010.0,
...
def test_idle_notification_needs_you(ws):
    session = fold(
        ws,
        event(ws, "Notification", notification_type="idle_prompt",
...
def test_notification_without_a_type_is_read_from_the_message(ws):
    session = fold(ws, event(ws, "Notification", message="Claude is waiting for your input"))
    assert session.reason == "waiting for input"
    session = fold(ws, event(ws, "Notification", message="Claude needs your permission"))
    assert session.state == "needs_you"
...
def test_auth_notification_does_not_need_you(ws):
    session = fold(
        ws,
        event(ws, "UserPromptSubmit", prompt="hi"),
        event(ws, "Notification", notification_type="auth_success", message="logged in"),
    )
    assert session.state == "working"
...
def test_needs_you_clears_on_the_next_prompt_or_tool_result(ws, clearing):
    session = fold(
        ws,
        event(ws, "Notification", notification_type="permission_prompt", ts=1010.0),
        event(ws, clearing, ts=1020.0, prompt="go on", tool_name="Bash",
...
def test_stop_is_done(ws):
    session = fold(ws, event(ws, "UserPromptSubmit", prompt="x"), event(ws, "Stop"))
...
def test_subagent_stop_keeps_the_state(ws):
    session = fold(
        ws,
        event(ws, "UserPromptSubmit", prompt="x"),
        event(ws, "SubagentStop", agent_type="Explore"),
    )
    assert session.state == "working"
...
def test_pre_compact_keeps_the_state_and_counts(ws):
    session = fold(
        ws,
        event(ws, "UserPromptSubmit", prompt="x"),
        event(ws, "PreCompact", trigger="auto"),
    )
    assert session.state == "working"
...
def test_session_end_is_ended(ws):
    session = fold(ws, event(ws, "Stop"), event(ws, "SessionEnd", reason="prompt_input_exit"))
    assert session.state == "ended"
    assert session.end_reason == "prompt_input_exit"


def test_unknown_event_does_not_crash(ws):
    session = fold(ws, event(ws, "SomethingNew", weird=1))
...
def test_a_session_whose_process_is_gone_is_dead(ws):
    session = fold(ws, event(ws, "UserPromptSubmit", prompt="x"))
...
def test_an_ended_session_stays_ended(ws):
    session = fold(ws, event(ws, "SessionEnd", reason="clear"))
    ws.mark_dead(session, alive=lambda pid: False)
    assert session.state == "ended"

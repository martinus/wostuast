...
| **Diff** | Changed files as a tree, and each file's diff in colour, with the changed words marked. Show it in one column or side by side. Pick what to show: all changes, only what is not committed, or one commit — step through the commits with **older** and **newer**. All changes has two halves, each saying what it is a diff of: everything this branch has committed that the default branch has not, and what the files on disk hold that the last commit does not. Untracked files are named on their own, because git has no diff for one. |
